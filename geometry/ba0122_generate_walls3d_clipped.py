...
<b>Uwaga:</b> do kontroli geometrii użyj „Rzut 2D 1:1”. Tryb „360° QA — równoległe ściany” używa projekcji afinicznej: przeciwległe krawędzie pozostają dokładnie równoległe i nie ma zbiegu perspektywicznego. Przy obrocie kwadrat może wyglądać jak równoległobok, ale nie powinien zwężać się jak trapez.<br>
...
<button onclick="setAffine360()">360° QA — równoległe ściany</button>
...
let rects=[], yaw=-0.75,pitch=0.58,zoom=70,drag=false,lx=0,ly=0,mode='affine';
let bounds=null;
...
function project(p){{
 ...
 if(mode==='affine'){{
   // Pure affine / cabinet-like architectural projection:
   // opposite plan edges stay exactly parallel; no perspective convergence.
   const sy=0.58;
   const zh=0.82;
   return [innerWidth/2+xr*zoom, innerHeight/2+yr*zoom*sy-z*zoom*zh, yr];
 }}
...
function setAffine360(){{mode='affine';pitch=0;zoom=Math.min(innerWidth,innerHeight)/12;draw();}}
